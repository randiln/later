...
export default function Capture() {
  ...
  const takePhoto = async () => {
    ...
    // Target resolution for Firestore storage (reasonable balance)
    ...
    if (ctx) {
      ...
      // Quality 0.7 is a good compromise for size vs quality (~100-200kb)
      const dataUrl = canvas.toDataURL("image/jpeg", 0.7);
      setPreview(dataUrl);

      // 2-second preview then clear and save
      setTimeout(async () => {
        try {
          setPreview(null);
          
          // Save directly to Firestore as Data URL (bypassing Storage timeouts)
          // Firestore document limit is 1MB, our compressed image is well below that.
          ...
          // Increment contributor shot count
          await updateDoc(doc(db, "galleries", id, "contributors", contributor.id), {
            shotsTaken: increment(1)
          });
        } catch (error: any) {
          console.error("Save process failed", error);
          const errorMessage = error?.message || "Failed to save photo";
          setError(errorMessage);
          
          if (error?.code?.includes('permission-denied') || errorMessage.includes('insufficient permissions')) {
            try {
              handleFirestoreError(error, OperationType.WRITE, `galleries/${id}/photos`);
            } catch (handledError) {
              // already handled
            }
          }
      ...
      <div className="relative flex-1 overflow-hidden bg-black flex items-center justify-center">
        {error ? (
          <div className="p-10 text-center space-y-4">
             ...
             <button onClick={startCamera} className="px-6 py-3 bg-white text-black rounded-full font-bold">Try Again</button>
          </div>
        ) : (
          <video 
            ref={videoRef} 
